...
from exporter.epub.ncx_maker import NcxMaker
from exporter.epub.opf_maker import OpfMaker
...
class EPubCreator:
    ...
    def append_chapter(self, title: str, paragraphs: List[str]):
        filename = f'chapter_{self.chapter_index}.html'
        with open(os.path.join(self.path, filename), 'w', encoding='utf-8') as ofile:
            ofile.write(
    ...
    def finish_book(self) -> str:
        ...
        output_filename = os.path.join(self.work_folder, f'{self.novel_name}.epub')
        epub_file = zipfile.ZipFile(output_filename, 'w', zipfile.ZIP_DEFLATED)
